...
public class ThreatKnowledgeProvider : MonoBehaviour, IThreatKnowledgeProvider
{
    ...
    private ThreatStaticKnowledgeBase staticKnowledgeBase;
    ...
    private ThreatEntityTypeKnowledge GetEntityTypeKnowledge(string entityType)
    {
        return staticKnowledgeBase.items
            .First(x => x.entityType == entityType)
            .knowledge;
    }
    ...
    private IEnumerable<PerceptionEntry> GetPerceptionsByEntity(Guid entityId)
    {
        return perceptionStorageRegistrar
            .GetPerceptionsFromRegisteredStorageForEntity(entityId)
            .Where(x => !x.PerceptionData.ContainsKey(PerceptionEntryCoreDataKeys.Construct));
    }
    ...
    private List<PerceptionEntry> GetActualPerceptions(
        IEnumerable<PerceptionEntry> perceptions)
    {
    ...
    private BoundCompoundProperty GetEntitySuspicion(
        IEnumerable<PerceptionEntry> perceptions)
    {
